...
# ── TTS ─────────────────────────────────────────────────────────────────
async def synthesize(sentence: str) -> tuple[bytes, int] | None:
    """Synthesize a sentence → raw (audio_bytes, samplerate). No playback."""
...
def play_blocking(data, samplerate: int) -> None:
    """Play audio in the current thread (blocking). Called via executor."""
...
async def process(user_input: str) -> None:
    ...
    Overlapped synthesize-and-play pipeline:
        ...
        sentence 1 → [synth]──────[play]
        sentence 2 →       [synth]──────[play]
        sentence 3 →             [synth]──────[play]
    ...
    # audio_future holds the synthesis task for the NEXT sentence,
    # started while the CURRENT sentence is playing.
    audio_future: asyncio.Task | None = None
    ...
    async for sentence in stream_sentences(user_input):
        ...
        # Start synthesizing THIS sentence immediately
        this_future = asyncio.create_task(synthesize(sentence))

        # If a previous synthesis is ready, play it now while this one synths
    ...
    # Play the final sentence (no next sentence to overlap with)
    if audio_future is not None:
        audio = await audio_future
        if audio:
            await loop.run_in_executor(None, play_blocking, *audio)
