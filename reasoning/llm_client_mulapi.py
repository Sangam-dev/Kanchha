...
COOLDOWN_SECS    = float(os.getenv("KEY_COOLDOWN_SECS", "60.0"))  # per-key cooldown on quota hit
    ...
    for attempt in range(MAX_RETRIES + 1):
        ...
        try:
            ...
            if _is_retryable(exc):
                pool.mark_quota(entry, exc)
                if attempt < MAX_RETRIES:
                    _log(f"[{model}] key[{entry.index}] quota hit — rotating key for stream")
                    continue
...
ALL_MODELS: list[str] = list(dict.fromkeys(
    [DEFAULT_MODEL] + _split_models(DEFAULT_FALLBACKS)
))
